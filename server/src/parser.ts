...
function parseCommand(commandSplit: commandSplit, diagnostics: Diagnostic[], textDocument: TextDocument, tokens: any[]): boolean {
	...
	while (iSubCommand < commandSplit.length) {
		...
		if (commandSplit[iSubCommand].subCommand in curDicCommand) {
			...
			if (commandSplit[iSubCommand].subCommand == "var") {
				tokens.push(addSemanticToken(textDocument,  commandSplit[iSubCommand - 1].indexStart, commandSplit[iSubCommand].indexEnd, "variable", [], true));
			}
		}
		else {
	...
	for (const actionType of typesVariablesPossible) {
		if (actionType.charAt(1) == "+") {
			...
			if (type == "var") {
				if (iStartVar + 2 >= commandSplit.length) {
					if (iStartVar + 1 >= commandSplit.length)
						...
						if (diagnostic == null){
							tokens.push(addSemanticToken(textDocument, commandSplit[iStartVar].indexStart, commandSplit[iStartVar].indexEnd, "variable", [], true));
			...
			if (type == "struct") {
				...
				if (isName.name == null)
					return { actionType: null, iEndVar: null, diagnostic: diagnosticUnexpectedExpression(commandSplit[iStartVar].indexStart, commandSplit[isName.iEndStruct].indexEnd, textDocument, "structure") }
